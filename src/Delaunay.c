...
int triang_compare(const void* p1, const void* p2) {
    return lex_compare(*(point*)p1, *(point*)p2);
}

Mesh triangulate(point* sites, unsigned int n) {
    Mesh edges;
    qsort(sites, n, sizeof *sites, triang_compare);
    edges = delaunay(sites, n);
    edges.vertices = sites;
    /* Find voronoi */
    return edges;
}

static Mesh delaunay(point* sites, unsigned int n) {
    if (n == 2) {
        Mesh edges;
...
Mesh make_ccw_triangle(point sites[3]) {
    Mesh edges;
    QuadEdge* a = make_edge(sites[0], sites[1]);
